...
public class PSPageStepDefinition extends UserBaseClass {
	...
	public void select_the_select_the_date_from_the_all_dates_job_ststus_from_all_status_regions_from_the_all_regions_search_driver_and_select_driver_from_the_all_drivers_vehicle_from_the_all_vehicle_and_select_the_sort_by(String string) throws Exception {
		...
		Thread.sleep(2000);
		sendKeys(ps.getSerchDriver(), "Sharma");
		Thread.sleep(2000);
		...
		clicks(ps.getVehicle1());
		clicks(ps.getVehicle2());
		clicks(ps.getVehicle3());
		escape();
	...
	@Given("Select a ready to pay status job, click on the Ready to pay button, Pay button, another CC, enter card name {string}, card number {string}, expiry date {string}, CVC {string} and click on the Make payment")
	public void select_a_ready_to_pay_status_job_click_on_the_ready_to_pay_button_pay_button_another_cc_enter_card_name_card_number_expiry_date_cvc_and_click_on_the_make_payment(String cname, String cnumber, String expirydate, String cvc) throws InterruptedException {
		ps = new PastJob();
		clicks(ps.getSelectJob());
		Thread.sleep(4000);
